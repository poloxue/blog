...
这种行情图抛弃了"时间" 的概念，专注于“纯粹的价格变动幅度”。我们需要的是预先设定一个固定的 "砖块大小"（比如股价的10元）。规则很简单：只有当价格运动超过一个砖块的大小，才会在图上形成一块新的砖；价格变动不足一个砖块时，图表会静止不动。
...
餐位管理逻辑，每单交易的最大损失固定为 2%（24 小时市场），止损单价格在下单价格损失 3 个 ATR，杠杆最大不超过 1.5。

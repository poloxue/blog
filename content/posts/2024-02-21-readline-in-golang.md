...
按行读取文件相较于一次性载入，有着很多优势，如内存效率高、处理速度快、实时性高、可扩展性强，特别是当遇到处理大文件时，这些优势会更加明显。
...
本文的重点在于如何使用 GO 实现按行读取，基于的是标准库的 `bufio.Reader` 和 `bufio.Scanner` 。而关于如何处理大文件的主题，我计划再写另外一篇文章重点介绍。

正式进入具体操作吧。

## 准备一个文本文件

先准备一个短小的文本文件 example.txt，内容如下：

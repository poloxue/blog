...
本文将介绍的 3 命令，用于提高 Web 程序员们的开发效率，

## 前言

对 Web 开发人员而言，除了基本的框架外，开发过程中，还常用到那必然是调试工具。本文将介绍的三个命令分别是 entr、httpie、jq。
...
这三个命令中每个命令都在 web 开发过程中扮演着不同的角色。开始具体介绍。
...
本文介绍了三个开发阶段常用的命令，分别是用于实时更新重启的命令 entr，更人性化的 http 客户端 httpie 和一款强大的 JSON 数据处理命令 jq。让它们 帮助提效我们的日常工作吧。

...
前面展示的都是利用标准库或三方库提供的能力，如果我们想自定义 tag 如何实现呢？毕竟有些情况下，如果默认提供的 tag 提供的能力不满足需求，我们还是希望可以自定义 tag 的行为。

这需要更深入地理解Go的反射机制，但它为数据处理和元信息管理提供了强大的灵活性。
...
在这个例子中，我们定义了一个自定义的tag `mytag`，并通过反射来访问它。
## 案例：结构体字段权限控制

让我们考虑一个实际的场景：一个权限系统。这个系统中，我们可以根据用户的角色（如 admin、user）控制对结构体字段的访问。具体而言，假设我定义了一个包含敏感信息的结构体，我可以使用自定义 tag 来标记每个字段的访问权限。
...
接下来就是要实现一个函数，实现根据 `UserProfile` 定义的 tag 权限决定是否显示字段内容。
...
权限控制的重点逻辑部分，就是 `if accessTag == "user" || accessTag == role`  这段判断条件。

### 使用演示

让我们来使用 `FilterFieldsByRole` 函数实现根据用户角色显示用户信息吧！

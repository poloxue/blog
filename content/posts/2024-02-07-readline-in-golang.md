...
按行进行文件读取是一项基础且常见的任务。在 Python 等语言中，按行读取文件非常直观和简单。

```python
```

好吧，我最喜欢拿 Python 作为对比语言，毕竟它最易用。

在 Go 中，文件读取虽不复杂，却有一些不同的方法。

本文将基于此展开讨论，介绍 Go 如何按行读取文件。这个问题想不明白，在处理大小文件的读取时，很容易影响效率。
